...
# ------------------- 添加 ViewComfy 安装和启动 -------------------
VIEWCOMFY_DIR="/workspace/ViewComfy"

echo "Installing ViewComfy..."

if [ ! -d "$VIEWCOMFY_DIR" ]; then
    git clone https://github.com/ViewComfy/ViewComfy.git "$VIEWCOMFY_DIR"
else
    echo "ViewComfy already exists, pulling latest..."
    cd "$VIEWCOMFY_DIR"
    git pull
    cd -
fi
...
# 安装依赖（需要 node.js，通常 Vast.ai 模板已预装或可 apt install）
npm install

# 构建生产版本（推荐，稳定）
npm run build

# 用 serve 启动在 8180 端口（serve 轻量、稳定）
# 先安装 serve（如果没装）
...
# 启动 ViewComfy 前端（连接本地 ComfyUI:8188）
COMFYUI_PORT=8188 serve -s dist -l 8180 --listen 0.0.0.0 &
echo "ViewComfy started on port 8180 (connecting to ComfyUI at :8188)"
